...
import tqdm
from data_loader import load_data_ranked, load_data
import argparse
import path_sampler
    ...
    for epoch in train_bar:
        ...
        with torch.no_grad():
            ...
            if max_val_acc < val_acc:
                ...
                y_hat = F.log_softmax(
                    predictor(X, neis[test_indices], num_w, walk_len, test_indices, path_type[test_indices], indxx),
                    dim=1)
...
for name in names:
    ...
    if name not in ['Bgp', 'Nba', 'Electronics']:
        (X, Y, num_classes, datasets, edge_index) = load_data_ranked(name)

    for i in range(rounds):
        print('round', i)
        if name in ['Bgp', 'Nba', 'Electronics']:
            (X, Y, num_classes, train_mask, val_mask, test_mask, edge_index) = load_data(name, i)
        else:
        ...
        deg = degree(torch.tensor(edge_index[0], dtype=torch.int64))
        deg = deg[edge_index[0]]
        rw_p = 1 / deg

        edge_index = edge_index.numpy().astype(np.int32)
        rw_p = rw_p.numpy().astype(np.float32)
        PS = path_sampler.PathSampler(edge_index, rw_p, num_of_walks, walk_length, 50)

        test_1f1, test_2f1, test_rec, test_prec, test_acc = train_fixed_indices(
            X, Y, num_classes, mode, name, train_mask, val_mask, test_mask, num_of_walks, hidden_size, walk_length, PS)
    ...
    print(
        mode + " Avg for {}: acc{:.4f} ± {:.4f}\t prec{:.4f} ± {:.4f}\t rec{:.4f} ± {:.4f}\t maf1{:.4f} ± {:.4f}\t mif1{:.4f} ± {:.4f}\t ".format(
            name, avg_test_acc, std_test_acc, avg_test_prec, std_test_prec, avg_test_rec, std_test_rec,
            avg_test_1f1, std_test_1f1, avg_test_2f1, std_test_2f1))
    print(
        mode + " Avg for {}: acc{:.4f} ± {:.4f}\t prec{:.4f} ± {:.4f}\t rec{:.4f} ± {:.4f}\t maf1{:.4f} ± {:.4f}\t mif1{:.4f} ± {:.4f}\t ".format(
            name, avg_test_acc, std_test_acc, avg_test_prec, std_test_prec, avg_test_rec, std_test_rec,
            avg_test_1f1, std_test_1f1, avg_test_2f1, std_test_2f1), file=file)
